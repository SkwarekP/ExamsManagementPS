...
export const Summary = ({exam}: Props) => {
    ...
    const isCorrectAnswer = result.filter((item) => mappedExamQuestionsById.filter((examQuestion) => item.answerId === examQuestion.questionId))
                        ...
                        {item.answers.map((opt) => (
                            <div
                                className={`${classes.option} 
                                    ${result && result[index].correctAnswer === opt
                                    ? classes.correctAnswer : (result && result[index].correctAnswer === opt) ? classes.incorrectAnswer : ""}`}
                            >
                                <input
                                    type="radio"
                                    disabled={true}
